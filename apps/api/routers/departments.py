...
async def get_department(
    ...
    department = await DepartmentService(session).get_department(
        department_id, tenant_id=current.tenant_id, actor_role=RoleName(current.role)
    )
...
async def create_department(
    ...
    department = await DepartmentService(session).create_department(
        tenant_id=current.tenant_id, actor_role=RoleName(current.role), name=payload.name
    )
    return DepartmentResponse.model_validate(department)
